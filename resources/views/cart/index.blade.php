...
// Create a new checkout function with different name
...
function loadOffers() {
    ...
    if (!window.isAuthenticated) {
        console.log('User not authenticated, skipping offers');
        return;
    }
    ...
    fetch('/offers/available')
        .then(response => {
            ...
            if (!contentType || !contentType.includes('application/json')) {
                console.log('Response is not JSON, skipping offers');
                return null;
            }
        ...
        .catch(error => {
            ...
            if (error.message.includes('Unexpected token') && error.message.includes('<!DOCTYPE')) {
                console.log('Server returned HTML instead of JSON, offers endpoint may not be available');
            } else {
                console.log('Error loading available offers (non-critical):', error.message);
            }
        });
    ...
    fetch('/offers/my-claims')
        .then(response => {
            if (!response.ok) {
                if (response.status === 401) {
                    console.log('User not authenticated, skipping claimed offers');
                    return null;
                }
                throw new Error(`HTTP ${response.status}: ${response.statusText}`);
            }
            ...
            const contentType = response.headers.get('content-type');
            if (!contentType || !contentType.includes('application/json')) {
                console.log('Response is not JSON, skipping claimed offers');
                return null;
            }
            return response.json();
        })
        ...
        .catch(error => {
            // Check if error is due to JSON parsing
            if (error.message.includes('Unexpected token') && error.message.includes('<!DOCTYPE')) {
                console.log('Server returned HTML instead of JSON, claimed offers endpoint may not be available');
            } else {
                console.log('Error loading claimed offers (non-critical):', error.message);
            }
        });
}
...
// Add missing offer functions to prevent errors
function claimOffer(offerCode, button) {
    console.log('Claiming offer:', offerCode);
    ...
    .then(data => {
        if (data.success) {
            ...
            showToast(data.message || 'Failed to claim offer', 'error');
            // Re-enable button
...
function applyClaimedOffer(claimId, button) {
    console.log('Applying claimed offer:', claimId);
    ...
    .catch(error => {
        console.error('Error applying offer:', error);
        showToast('Error applying offer. Please try again.', 'error');
        // Re-enable button
        button.disabled = false;
        button.textContent = 'Apply';
    });
...
// Add missing clearCart function
function clearCart() {
