...
<body class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-2 sm:py-4 md:py-8 lg:py-12">
                    ...
                    <div class="bg-white bg-opacity-90 backdrop-blur-sm rounded-xl p-3 sm:p-4 md:p-6 shadow-lg">
                        <div class="flex flex-col sm:flex-row justify-between items-center mb-3 gap-2">
                            ...
                            <div class="text-center sm:text-right">
                                <div class="text-xl sm:text-2xl md:text-3xl font-bold text-green-600">₹{{ number_format($stats['total_invested'] ?? 0) }}</div>
                        ...
                        <div class="relative">
                            <div class="w-full bg-gray-200 rounded-full h-3 sm:h-4 md:h-6">
                                ...
                                @endphp
                                <div class="bg-gradient-to-r from-green-500 to-green-600 h-3 sm:h-4 md:h-6 rounded-full transition-all duration-500" style="width: {{ $percentage }}%"></div>
                                @if($excessPercentage > 0)
                                    <div class="bg-gradient-to-r from-blue-500 to-purple-600 h-3 sm:h-4 md:h-6 rounded-full transition-all duration-500" style="width: {{ $excessPercentage }}%; margin-left: 100%;"></div>
                                @endif
                            </div>
                            <div class="flex flex-col sm:flex-row justify-between items-center mt-2 gap-1">
                                ...
                                <div class="text-xs sm:text-sm font-medium">
                                    @if($raised >= $target)
                                        <span class="text-green-600">Target Achieved! +{{ number_format($excessPercentage, 1) }}%</span>
                                    @else
                                        <span class="text-blue-600">{{ number_format($percentage, 1) }}%</span>
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <div class="bg-white rounded-xl md:rounded-2xl shadow-xl p-3 sm:p-4 md:p-8 mb-4 sm:mb-6 md:mb-12">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3 md:gap-6 mb-3 sm:mb-4 md:mb-8">
                <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg md:rounded-xl shadow-lg p-2 sm:p-3 md:p-6 text-center text-white">
                    <div class="text-lg sm:text-xl md:text-3xl font-bold">{{ $stats['total_investors'] ?? 0 }}</div>
                ...
                <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-lg md:rounded-xl shadow-lg p-2 sm:p-3 md:p-6 text-center text-white">
                    <div class="text-sm sm:text-lg md:text-3xl font-bold">₹{{ number_format($stats['total_invested'] ?? 0) }}</div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="{{ $rowColor }} transition-colors">
                                    <td class="px-1 sm:px-2 md:px-6 py-1 sm:py-2 md:py-4 whitespace-nowrap">
                                        <div class="text-xs sm:text-sm font-medium text-gray-900">{{ $serialNumber }}</div>
                                    </td>
                                    <td class="px-1 sm:px-2 md:px-6 py-1 sm:py-2 md:py-4 whitespace-nowrap">
                                        <div class="text-xs sm:text-sm font-medium text-gray-900">{{ $investor->name ?? $investor->full_name ?? 'N/A' }}</div>
                                    </td>
                                    <td class="px-1 sm:px-2 md:px-6 py-1 sm:py-2 md:py-4 whitespace-nowrap">
                                        <div class="text-xs sm:text-sm font-medium text-gray-900">₹{{ number_format($investor->investment_amount ?? $investor->amount ?? $investor->total_invested ?? 0) }}</div>
                                    </td>
                                    <td class="px-1 sm:px-2 md:px-6 py-1 sm:py-2 md:py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-1 sm:px-2 md:px-6 py-1 sm:py-2 md:py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center px-1 sm:px-1.5 md:px-2.5 py-0.5 rounded-full text-xs font-medium {{ $likelihoodColors[$likelihood] }}">
                                            {{ $likelihood }}/5
                                        </span>
                                    </td>
                                </tr>
                    ...
                    <div class="mb-3 sm:mb-4 md:mb-6">
                        ...
                        <div class="flex items-center gap-1 md:gap-2">
                            @for ($i = 1; $i <= 5; $i++)
                                ...
                                <label for="likelihood-{{ $i }}" class="text-xl sm:text-2xl md:text-3xl cursor-pointer text-gray-300 hover:text-yellow-400 transition-all duration-300 hover:scale-125 star-label" data-value="{{ $i }}">
                                    ★
                                </label>
                            @endfor
                        </div>
    ...
    function downloadPDF() {
        try {
            ...
            link.href = '{{ asset("storage/documents/investment-guide.pdf") }}';
