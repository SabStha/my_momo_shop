{{-- BOTTOM NAVBAR --}}
<nav class="fixed bottom-0 left-0 right-0 z-50 text-white flex justify-around items-center py-1.5 shadow-lg" style="background-color: #152039; box-shadow: 0 2px 5px rgba(0,0,0,.08);">
    @php
        $navItems = [
    ...
    @foreach ($navItems as $item)
        <a href="{{ route($item['route']) }}"
           class="flex flex-col items-center text-xs transition-colors active:scale-95 {{ request()->routeIs($item['route'].'*') ? 'text-amk-gold' : 'text-white hover:text-white' }}">
