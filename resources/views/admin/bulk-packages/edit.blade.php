...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Edit Bulk Package</h1>
            <a href="{{ route('admin.bulk-packages.index') }}" class="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700 transition">
                Back to Packages
            </a>
        </div>

        <form action="{{ route('admin.bulk-packages.update', $bulkPackage) }}" method="POST" enctype="multipart/form-data" x-data="packageForm()" @submit="console.log('Form submitted', $data)">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white shadow-md rounded-lg p-6 space-y-6">
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $bulkPackage->name) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="emoji" value="{{ old('emoji', $bulkPackage->emoji) }}" required placeholder="🎉"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                        @error('emoji')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" required
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $bulkPackage->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <select name="type" required
                            ...
                            <option value="cooked" {{ old('type', $bulkPackage->type) === 'cooked' ? 'selected' : '' }}>Cooked</option>
                            <option value="frozen" {{ old('type', $bulkPackage->type) === 'frozen' ? 'selected' : '' }}>Frozen</option>
                        </select>
                        @error('type')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="package_key" value="{{ old('package_key', $bulkPackage->package_key) }}" required placeholder="party"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                        @error('package_key')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', $bulkPackage->sort_order) }}" min="0"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                        @error('sort_order')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
                    ...
                    <div class="space-y-4">
                        @if($bulkPackage->image)
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Current Image</label>
                            <div class="relative inline-block">
                                <img src="{{ '/storage/' . $bulkPackage->image }}" alt="Current Package Image" class="w-32 h-24 object-cover rounded-lg border border-gray-300">
                            </div>
                        </div>
                        @endif
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="deal_title" value="{{ old('deal_title', $bulkPackage->deal_title) }}" placeholder="Party Pack Deal"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            @error('deal_title')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" name="feeds_people" value="{{ old('feeds_people', $bulkPackage->feeds_people) }}" placeholder="8–10 people"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            @error('feeds_people')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" name="savings_description" value="{{ old('savings_description', $bulkPackage->savings_description) }}" placeholder="Save Rs. 250+ vs buying individually"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <input type="number" name="original_price" x-model="totalPrice" step="0.01" readonly
                                   value="{{ old('original_price', $bulkPackage->original_price) }}" placeholder="2567.00"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="delivery_note" value="{{ old('delivery_note', $bulkPackage->delivery_note) }}" placeholder="Order before 2PM for same-day delivery"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            @error('delivery_note')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="total_price" x-model="totalPrice" step="0.01" required
                                   value="{{ old('total_price', $bulkPackage->total_price) }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="bulk_price" x-model="totalBulkPrice" step="0.01" required readonly
                                   value="{{ old('bulk_price', $bulkPackage->bulk_price) }}"
...
<script>
    function packageForm() {
        const initialItems = @json(old('items', $bulkPackage->items ?? []));
        ...
        return {
            ...
            init() {
                // Ensure items have quantity and bulk_price properties
                this.items = this.items.map(item => ({
                    ...item,
                    quantity: item.quantity || 1,
                    bulk_price: item.bulk_price || item.price || 0
                }));
            ...
            updateTotalPriceField() {
                ...
                // Update values without triggering form events
                if (totalPriceInput) {
            ...
            addProductToPackage(product) {
                ...
                // Only update the display, don't trigger form submission
                this.$nextTick(() => {
                    this.updateTotalPriceField();
                });
            },
