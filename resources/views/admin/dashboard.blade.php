    ...
    @if(session('selected_branch_id'))
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div class="text-center">
                        <div class="text-2xl font-bold text-indigo-600">{{ $salesAnalytics['current_month']['orders'] }}</div>
                        <div class="text-xs text-gray-500">Orders</div>
                        <div class="text-xs {{ $salesAnalytics['growth']['orders'] >= 0 ? 'text-green-600' : 'text-red-600' }}">
                            {{ $salesAnalytics['growth']['orders'] >= 0 ? '+' : '' }}{{ number_format($salesAnalytics['growth']['orders'], 1) }}%
                                    </div>
                                </div>
                    <div class="text-center">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="text-center p-4 bg-blue-50 rounded-lg">
                        <div class="text-2xl font-bold text-blue-600">{{ $campaignMetrics['total_redemptions'] }}</div>
                        <div class="text-sm text-gray-600">Total Redemptions</div>
                    </div>
                    <div class="text-center p-4 bg-green-50 rounded-lg">
                        <div class="text-2xl font-bold text-green-600">{{ number_format($campaignMetrics['average_open_rate'], 1) }}%</div>
                        <div class="text-sm text-gray-600">Avg Open Rate</div>
                    </div>
                    <div class="text-center p-4 bg-yellow-50 rounded-lg">
                        <div class="text-2xl font-bold text-yellow-600">{{ number_format($campaignMetrics['average_engagement_rate'], 1) }}%</div>
                        <div class="text-sm text-gray-600">Avg Engagement</div>
                    </div>
                    <div class="text-center p-4 bg-purple-50 rounded-lg">
                        <div class="text-2xl font-bold {{ $campaignMetrics['average_roi'] >= 0 ? 'text-green-600' : 'text-red-600' }}">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <a href="{{ route('admin.campaigns.create') }}" class="text-sm text-indigo-600 hover:text-indigo-900">Create New</a>
                </div>
                <div class="space-y-3">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('admin.rules.create') }}" class="text-sm text-indigo-600 hover:text-indigo-900">Create New Rule</a>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @forelse($rules->take(6) as $rule)
                    <div class="border rounded-lg p-4">
                        ...
                        <p class="text-sm text-gray-500 mb-2">{{ Str::limit($rule->description, 60) }}</p>
                        <div class="flex justify-between items-center">
                            <span class="text-xs text-gray-500">Priority: {{ $rule->priority }}</span>
                            <a href="{{ route('admin.rules.edit', $rule) }}" class="text-xs text-indigo-600 hover:text-indigo-900">Edit</a>
                        </div>
                    </div>
                @empty
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (salesByHourCtx) {
            new Chart(salesByHourCtx.getContext('2d'), {
                type: 'bar',
                data: {
                    labels: {!! json_encode($salesByHour->pluck('hour')->map(function($hour) { return $hour . ':00'; }) ?? []) !!},
                    datasets: [{
                        label: 'Revenue',
                        data: {!! json_encode($salesByHour->pluck('total_amount') ?? []) !!},
        ...
        if (productCategoriesCtx) {
            new Chart(productCategoriesCtx.getContext('2d'), {
                type: 'pie',
            data: {
                    labels: {!! json_encode($productCategories->pluck('category') ?? []) !!},
                datasets: [{
                        data: {!! json_encode($productCategories->pluck('total_revenue') ?? []) !!},
                        backgroundColor: [
                            'rgba(79, 70, 229, 0.8)',
                            'rgba(16, 185, 129, 0.8)',
                            'rgba(245, 158, 11, 0.8)',
        ...
        if (customerSegmentsCtx) {
            new Chart(customerSegmentsCtx.getContext('2d'), {
                ...
                data: {
                    ...
                    datasets: [{
                        data: {!! json_encode($customerSegments->pluck('count') ?? []) !!},
                        backgroundColor: [
                            'rgba(16, 185, 129, 0.8)',
                            'rgba(245, 158, 11, 0.8)',
                            'rgba(239, 68, 68, 0.8)',
        ...
        if (revenueDistributionCtx) {
            new Chart(revenueDistributionCtx.getContext('2d'), {
                type: 'polarArea',
                data: {
                    labels: {!! json_encode($revenueDistribution->pluck('day') ?? []) !!},
                    datasets: [{
                        data: {!! json_encode($revenueDistribution->pluck('revenue') ?? []) !!},
                        backgroundColor: [
                            'rgba(79, 70, 229, 0.6)',
                            'rgba(16, 185, 129, 0.6)',
                            'rgba(245, 158, 11, 0.6)',
