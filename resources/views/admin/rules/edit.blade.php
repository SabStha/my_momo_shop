...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-semibold text-gray-900">Edit Automation Rule: {{ $rule->name }}</h1>
                    ...
                    <div>
                        ...
                        <div class="mt-4 grid grid-cols-1 gap-4">
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $rule->name) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                                @error('name')
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('description', $rule->description) }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="priority" id="priority" value="{{ old('priority', $rule->priority) }}" min="1" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                                @error('priority')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <div class="mt-2">
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $rule->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
...
const initialConditionsRaw = {!! json_encode(old('conditions', $rule->conditions ?? [])) !!};
const initialConditions = Array.isArray(initialConditionsRaw) ? initialConditionsRaw : Object.values(initialConditionsRaw || {});

const initialActionsRaw = {!! json_encode(old('actions', $rule->actions ?? [])) !!};
...
// Render dynamic fields based on current data
