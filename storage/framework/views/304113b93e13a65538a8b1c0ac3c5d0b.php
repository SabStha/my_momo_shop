...
<div class="container" style="max-width: 700px; margin: 2rem auto;">
    ...
    <?php if(count($cart) > 0): ?>
        <div class="cart-items" style="display: flex; flex-direction: column; gap: 1.2rem;">
            <?php $__currentLoopData = $cart; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="cart-item" style="display: flex; align-items: center; background: #fff; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); padding: 1rem; gap: 1rem;">
                    <img src="<?php echo e(asset('storage/' . ($item['image'] ?? 'default-image.jpg'))); ?>" alt="<?php echo e($item['name'] ?? 'Product'); ?>" style="width: 80px; height: 80px; object-fit: cover; border-radius: 8px; border: 1px solid #eee;">
                    <div class="item-details" style="flex: 1;">
                        <h3 style="margin: 0 0 0.3rem 0; font-size: 1.1rem; font-weight: 600;"><?php echo e($item['name'] ?? 'Unnamed Product'); ?></h3>
                        <p class="price" style="margin: 0 0 0.5rem 0; color: #c1440e; font-weight: 500;">$<?php echo e(number_format($item['price'] ?? 0, 2)); ?></p>
                        <div class="quantity" style="display: flex; align-items: center; gap: 0.5rem;">
                            <form action="<?php echo e(route('cart.update', ['id' => $item['product_id'] ?? ''])); ?>" method="POST" style="display: flex; align-items: center; gap: 0.3rem;">
                                <?php echo csrf_field(); ?>
                                <input type="hidden" name="product_id" value="<?php echo e($item['product_id'] ?? ''); ?>">
                                <button type="submit" name="quantity" value="<?php echo e(max(1, ($item['quantity'] ?? 1) - 1)); ?>" style="background: #eee; border: none; border-radius: 50%; width: 28px; height: 28px; font-size: 1.1rem;">-</button>
                                <input type="number" name="quantity" value="<?php echo e($item['quantity'] ?? 1); ?>" min="1" style="width: 40px; text-align: center; border: 1px solid #e8d9cc; border-radius: 6px; padding: 2px 0;">
                                <button type="submit" name="quantity" value="<?php echo e(($item['quantity'] ?? 1) + 1); ?>" style="background: #eee; border: none; border-radius: 50%; width: 28px; height: 28px; font-size: 1.1rem;">+</button>
                            </form>
                        </div>
                    </div>
                    <form action="<?php echo e(route('cart.remove', ['id' => $item['product_id'] ?? ''])); ?>" method="POST" style="margin-left: 0.5rem;">
                        <?php echo csrf_field(); ?>
                        <input type="hidden" name="product_id" value="<?php echo e($item['product_id'] ?? ''); ?>">
        ...
        <div class="cart-summary" style="margin-top: 2rem; background: #fffaf3; border-radius: 10px; box-shadow: 0 1px 4px rgba(0,0,0,0.04); padding: 1.2rem; text-align: right;">
            <h3 style="margin: 0 0 1rem 0; font-size: 1.3rem; font-weight: 700; color: #6e3d1b;">Total: $<?php echo e(number_format($total ?? 0, 2)); ?></h3>
    ...
    <?php else: ?>
        <div style="text-align: center; margin: 3rem 0; color: #aaa;">
            ...
            <a href="<?php echo e(route('home')); ?>" class="btn" style="background: #c1440e; color: #fff; border-radius: 8px; padding: 0.7rem 2rem; font-weight: 600; text-decoration: none;">Continue Shopping</a>
        </div>
    <?php endif; ?>
</div>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\sabst\momo_shop\resources\views/desktop/cart.blade.php ENDPATH**/ ?>